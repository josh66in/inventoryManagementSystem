...
public class DatabaseDao {
	...
	private Float totalValue = new Float(0.0);
	private Float netProfitOrLossSinceLastReportGeneration = new Float(0.0);
	
	// Can be made synchronizedhashmap or concurrenthashmap as need be for thread
	// safety.
	// simulates the database. hence there it is static as the database is only one.
	public static Map<ItemNameUnit, List<InventoryDataUnit>> dataBase = new HashMap<>();

	public static Map<ItemNameUnit, List<InventoryDataUnit>> getDataBase() {
		return dataBase;
	}

	// Creates hashmap with arraylist as value which holds all the price changes.
	// The question statement mentioned two but
	...
	public boolean create(String itemName, Float costPrice, Float sellingPrice) {
		...
		InventoryDataUnit idu = new InventoryDataUnit(costPrice, sellingPrice, new Integer(0), new Integer(0),
				new Float(0));
		List<InventoryDataUnit> iduList = new ArrayList<>();
		iduList.add(idu);
	...
	public boolean updateSell(String itemName, Integer quantity) {
		...
		// updates value for profit or loss since last report. When the report is
		// generated this is zeroed.
		idu.setProfitOrLossSinceLastReport(
				idu.getProfitOrLossSinceLastReport() + (quantity * (idu.getSellingPrice() - idu.getCostPrice())));
		// updates value for qty since last report and when report is generated this is
		// zeroed.
	...
	// adds a new element to arraylist and zeros the previously held last element's
	// qty to zero
	public boolean updateSellPrice(String itemName, Float newSellPrice) {
		...
		InventoryDataUnit iduNew = new InventoryDataUnit(idu.getCostPrice(), newSellPrice, idu.getCurrentQuantity(),
	...
	public void report() {
		...
		Map<ItemNameUnit, List<InventoryDataUnit>> sortedDataBase = new TreeMap<>();

		sortedDataBase = new TreeMap<>(dataBase);
		...
		while (itr.hasNext()) {
			...
			while (itr1.hasNext()) {
				...
				else if (iduList.size() == 1 || idu.getProfitOrLossSinceLastReport() != 0
						|| idu.getQuantitySoldSinceLastReport() != 0) {

					System.out.println(inu.getItemName() + "\t\t\t" + df.format(idu.getCostPrice()) + "\t\t\t\t"
							+ df.format(idu.getSellingPrice()) + "\t\t\t\t" + df1.format(idu.getCurrentQuantity())
		...
		this.netProfitOrLossSinceLastReportGeneration = profitOrLossSinceLastReportGeneration -
				costPriceOfDeletedItems;
		System.out.println("---------" + "------------" + "---------" + "----------------" + "-------"
				+ "----------------" + "------------" + "------------" + "-----");
		System.out.println("Total Value" + "\t\t\t\t" + "\t\t\t\t" + "\t\t\t\t" + df.format(this.totalValue));
		System.out.println("Profit since previous report" + "\t\t\t\t" + "\t\t\t\t" + "\t\t\t\t"
				+ df.format(this.netProfitOrLossSinceLastReportGeneration));
	...
	public Float getNetProfitOrLossSinceLastReportGeneration() {
		return netProfitOrLossSinceLastReportGeneration;
	}
